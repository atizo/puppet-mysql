...
Puppet::Type.type(:mysql_grant).provide(:mysql) do
  ...
  def row_exists?
    ...
    not mysql("mysql", "-NBe", 'SELECT "1" FROM %s WHERE %s' % [ 
      name[:type], fields.map { |f| "%s = '%s'" % [f, name[f]] }.join(' AND ')
    ]).empty?
  end
  ...
  def privileges 
    ...
    case name[:type]
    when :user
      privs = mysql "mysql", "-Be", 'select * from user where user="%s" and host="%s"' % [
        name[:user], name[:host]
      ]
    when :db
      privs = mysql "mysql", "-Be", 'select * from db where user="%s" and host="%s" and db="%s"' % [
